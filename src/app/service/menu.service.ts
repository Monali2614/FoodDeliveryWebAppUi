...
export class MenuService {
  ...
  addMenu(restaurantId: number, menuItem: Menu): Observable<Menu> {
    ...
    console.log('Sending menu item:', menuItem); // Log the menuItem object
  ...
  searchMenuItem(itemName: string): Observable<Restaurant[]> {
    return this.http.get<Restaurant[]>(`${NAV_URL}/api/menus/menu/by-menu/${encodeURIComponent(itemName)}`)
      .pipe(catchError(this.handleError));
  }
